...
sub ref2TextNode {
  ...
  if ( ref($param{data}) !~ /XML::LibXML/ ) {
    $param{data} = $self->convert(%param, depth => $param{depth}+1);
...
sub getConverter {
  ...
  $param{encoding} ||= '';
  if ( $param{encoding} eq $self->toEncoding ) {
    return undef;
  }
...
sub convert {
  ...
  # if noConvert is on return instantly.
  return $param{data} if $param{noConvert};

  my $prev = $self->showPath;
  $self->showPath($param{showpath}) if exists $param{showpath};
  warn "-" x $param{depth}, "convert\n" if $self->showPath;

  # convert if you can.
  my $iconv = $self->getConverter(%param);
  $param{data} = '' if not defined $param{data};

  if  ( $iconv ) {
    $param{data} = $iconv->convert($param{data});
  }
...
=head2 objectIconv ( bless({},"Text::Iconv") )
